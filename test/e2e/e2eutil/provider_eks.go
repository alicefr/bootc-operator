...
func NewEKSProvider(
	...
	ltID, ltVersion, err := discoverLaunchTemplateFromASG(
		ctx, asgClient, ec2Client, asgName,
	)
	if err != nil {
		return nil, err
	}
...
func discoverLaunchTemplateFromASG(
	ctx context.Context,
	asgClient *autoscaling.Client,
	ec2Client ec2DescribeLaunchTemplateVersionsAPI,
	asgName string,
) (ltID, ltVersion string, err error) {
	asgOut, err := asgClient.DescribeAutoScalingGroups(
		ctx,
		&autoscaling.DescribeAutoScalingGroupsInput{
	...
	if err != nil {
		return "", "", fmt.Errorf("describing ASG %q: %w", asgName, err)
	}
	...
	if v == nil {
		return "", "", fmt.Errorf("launch template version number is nil")
	}
	return ltID, fmt.Sprintf("%d", *v), nil
}
